...
namespace TestCommon
{
    public static class TestTools
    {
            ...
            Func<long, long[], long[], long[,]> longProcessor)
        {
            ...
            firstLine = ReadParallelArrayTotal(inStr, list1, list2);

            long[,] res = longProcessor(firstLine,
                list1.ToArray(),
                list2.ToArray());
		...
		private static long ReadParallelArrayTotal(string inStr,
            List<long> list1, List<long> list2)
        {
            long firstLine;
            using (StringReader reader = new StringReader(inStr))
            {
                ...
                var toks = line.Split(IgnoreChars, StringSplitOptions.RemoveEmptyEntries);
                foreach (string x in toks)
                {
                    list1.Add(Convert.ToInt64(x));
                }

                line = reader.ReadLine();
                toks = line.Split(IgnoreChars, StringSplitOptions.RemoveEmptyEntries);
                foreach (string x in toks)
                {
                    list2.Add(Convert.ToInt64(x));
                }
            }
